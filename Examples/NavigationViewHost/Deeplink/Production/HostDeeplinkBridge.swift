...
/// the feature is presented modally over the legacy hierarchy, so a deeplink
/// that navigates underneath it would be lost. The bridge holds the URL until
/// the screen is clear, then hands it to the existing handler.
final class HostDeeplinkBridge: ObservableObject {
    ...
    /// - Parameters:
    ///   - presenter: the feature's presentation, closed before a deeplink
    ///     navigates.
    ...
    ///
    /// Closes the feature if it is on screen and waits for the dismissal
    /// before handing the URL on - UIKit reports that moment exactly.
    @discardableResult
    ...
    /// Hands the pending URL to the app's own handler, once the screen is
    /// clear.
