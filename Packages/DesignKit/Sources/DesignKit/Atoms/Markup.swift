...
enum MarkupParser {
    ...
    private struct Frame {
        ...
        mutating func append(text: String) {
            ...
            runs.append(
                MarkupRun(text: text, style: MarkupStyle())
            )
        }

        mutating func append(runs newRuns: [MarkupRun]) {
            runs.append(contentsOf: newRuns)
        }

        /// Applies this frame's tag to everything it collected.
        func styledRuns() -> [MarkupRun] {
            guard let tag else {
                return runs
            }
            switch tag {
        ...
        private func apply(
            _ run: MarkupRun,
            _ change: (inout MarkupStyle) -> Void
        ) -> MarkupRun {
            var updated = run
