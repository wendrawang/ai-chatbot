...
final class TanyaAIImageCache {
    static let shared = TanyaAIImageCache()
...
struct TanyaAIRemoteImage: View {
    private let url: URL?
    ...
    init(url: URL?, aspectRatio: Double) {
        ...
        _image = State(initialValue: TanyaAIImageCache.shared.image(for: url))
    }
    ...
    private func load() {
        ...
        URLSession.shared.dataTask(with: url) { data, _, _ in
            ...
            TanyaAIImageCache.shared.store(loaded, for: url)
...
/// it never does.
private struct TanyaAIImagePlaceholder: View {
